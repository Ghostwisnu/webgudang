...
class M_admin extends CI_Model
{
  ...
  public function menambahkan($tabel,$id_transaksi,$jumlah)
  {
    $this->db->set("jumlah","jumlah + $jumlah");
    $this->db->where('id_transaksi',$id_transaksi);
    $this->db->update($tabel);
  }
  public function perkalian($tabel,$id_transaksi,$jumlah)
  {
    $this->db->set("jumlah","jumlah * $jumlah");
    $this->db->where('id_transaksi',$id_transaksi);
    $this->db->update($tabel);
  ...
  public function get_products_by_name($po) {
        $this->db->like('po_number', $po, 'both'); // Cari berdasarkan nama produk
        $this->db->limit(10); // Batasi hasil pencarian
        $query = $this->db->get('tb_po'); // Ganti 'products' dengan nama tabel Anda
        return $query->result();
    }
...
//////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////// TRIAL AUTOFILL
     public function get_po_data($po_number) {
        return $this->db->get_where('tb_po_number', ['po_number' => $po_number])->row();
    }

    public function get_all_po_numbers() {
        return $this->db->get('tb_po_number')->result();
    }
}
